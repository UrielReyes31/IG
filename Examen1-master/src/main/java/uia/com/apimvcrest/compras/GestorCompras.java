    ...
    public GestorCompras() throws IOException {
        ...
        for (Entry<Integer, HashMap<Integer, ArrayList<InfoComprasUIA>>> item : miComprador.getSolicitudesOrdenCompraAgrupadosXvendedores().entrySet()) {
            ...
            for (Entry<Integer, ArrayList<InfoComprasUIA>> soc : nodo.entrySet()) {
                ...
                mapper.writeValue(new File("C:\\Users\\uriel\\IdeaProjects\\Examen1-master\\SolicitudOrdenCompra-" + newSolicitud.getName() + ".json"), newSolicitud);
            }
        }
            ...
            if (misCotizacionesOrdenCompra.get(i).getItems() != null) {
                ...
                for (int j = 0; j < misCotizacionesOrdenCompra.get(i).getItems().size(); j++) {
                    ...
                    ItemCotizacionModelo nodo = new ItemCotizacionModelo(
                            ...
                            ,misCotizacionesOrdenCompra.get(i).getItems().get(j).getExistencia_inicial()
                            ,misCotizacionesOrdenCompra.get(i).getItems().get(j).getExistencia_minima()
                            ,misCotizacionesOrdenCompra.get(i).getItems().get(j).getConsumo_mensual()
                    );
                ...
                if (nodo.getValue().getItems() != null)
                {
                    ...
                    for (int j = 0; j < nodo.getValue().getItems().size(); j++) {
                        //ItemCotizacionModelo(int cantidad, double valorUnitario, double subtotal, double total)
                                 ...
                                 nodo.getValue().getItems().get(j).setName("sacapuntas");
                                 nodo.getValue().getItems().get(j).setDescripcion("sacapuntas marca ABC");
                                 nodo.getValue().getItems().get(j).setExistencia_inicial(50);
                                 nodo.getValue().getItems().get(j).setExistencia_minima(30);
                                 nodo.getValue().getItems().get(j).setConsumo_mensual(15);
